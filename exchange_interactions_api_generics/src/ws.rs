...
pub enum WsError {
	...
	#[diagnostic(code(exchange_interactions::ws::tungstenite), help("WebSocket protocol error. The connection may need to be reestablished."))]
	Tungstenite(tungstenite::Error),
	#[diagnostic(transparent)]
	Auth(ConstructAuthError),
	#[diagnostic(code(exchange_interactions::ws::parse), help("Failed to parse WebSocket message. Check if the exchange API has changed."))]
	Parse(serde_json::Error),
	#[diagnostic(code(exchange_interactions::ws::subscription))]
	Subscription(String),
	#[diagnostic(code(exchange_interactions::ws::network), help("Network connection failed. Check your internet connection."))]
	NetworkConnection,
	#[diagnostic(transparent)]
	Url(UrlError),
	#[diagnostic(
		code(exchange_interactions::ws::unexpected_event),
		help("Received an unexpected event from the WebSocket. This may indicate an API change.")
	)]
