...
pub enum ConstructAuthError {
	...
	#[diagnostic(code(exchange_interactions::auth::missing_pubkey), help("Provide API public key in your credentials"))]
	MissingPubkey {
		#[new(value = "Backtrace::capture()")]
		backtrace: Backtrace,
	},
	#[error("missing API secret key")]
	#[diagnostic(code(exchange_interactions::auth::missing_secret), help("Provide API secret key in your credentials"))]
	MissingSecret {
		#[new(value = "Backtrace::capture()")]
		backtrace: Backtrace,
	},
	#[error("invalid character in API key: {key}")]
	#[diagnostic(code(exchange_interactions::auth::invalid_api_key))]
...
pub enum UrlError {
	...
	#[diagnostic(code(exchange_interactions::url::parse))]
	Parse(#[from] url::ParseError),
	#[error("Exchange does not provide testnet for requested endpoint: {0}")]
	#[diagnostic(
		code(exchange_interactions::url::missing_testnet),
