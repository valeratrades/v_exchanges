...
use exchange_interactions_adapters::binance::{BinanceAuth, BinanceHttpUrl, BinanceOption};
...
pub async fn income_history(client: &exchange_interactions_adapters::Client, request: IncomeRequest, recv_window: Option<std::time::Duration>) -> ExchangeResult<Vec<IncomeRecord>> {
...
pub(in crate::binance) async fn personal_info(
	client: &exchange_interactions_adapters::Client,
	recv_window: Option<std::time::Duration>,
	prices: &BTreeMap<Pair, f64>,
) -> ExchangeResult<PersonalInfo> {
	assert!(client.is_authenticated::<BinanceOption>());
