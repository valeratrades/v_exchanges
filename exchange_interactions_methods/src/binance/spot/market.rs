...
pub async fn prices(client: &exchange_interactions_adapters::Client, pairs: Option<Vec<Pair>>) -> ExchangeResult<BTreeMap<Pair, f64>> {
...
pub async fn exchange_info(client: &exchange_interactions_adapters::Client) -> ExchangeResult<ExchangeInfo> {
	let options = vec![BinanceOption::HttpUrl(BinanceHttpUrl::Spot)];
